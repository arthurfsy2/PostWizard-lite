...
function TabBar({
  ...
  const t = useTranslations('SentReply');
  const tabs = [
    { id: "timeline" as TabType, icon: List, label: t('tabTimeline') },
    { id: "wordcloud" as TabType, icon: Cloud, label: t('tabWordcloud') },
    { id: "highlights" as TabType, icon: MessageSquare, label: t('tabHighlights') },
  ];
...
function OnboardingGuide({ onStartSearch }: { onStartSearch: () => void }) {
  const t = useTranslations('SentReply');
  const steps = [
    { icon: Mail, title: t('step1Title'), desc: t('step1Desc') },
    { icon: Search, title: t('step2Title'), desc: t('step2Desc') },
    { icon: Compass, title: t('step3Title'), desc: t('step3Desc') },
  ];
  ...
  return (
    <div className="my-4 overflow-hidden rounded-[28px] border border-white/70 bg-white/90 shadow-xl shadow-slate-900/5">
      <div className="bg-gradient-to-r from-orange-500/8 via-amber-500/6 to-emerald-500/8 px-6 py-6 sm:px-8">
        <h3 className="text-center text-xl font-bold text-slate-900">{t('welcomeTitle')}</h3>
      ...
      {!isLoading && hasData && (
        <div className="mb-5 overflow-hidden rounded-[26px] border border-white/70 bg-white/85 p-4 shadow-lg shadow-slate-900/5 sm:p-5">
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-semibold text-slate-900">{t('countryFilter')}</div>
              <div className="text-xs text-slate-500">{t('countryFilterDesc')}</div>
            </div>
            {selectedCountry && (
              <button
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-orange-100 bg-white/90 px-4 py-3 shadow-sm shadow-orange-500/10">
                  <div className="text-xs font-medium text-slate-500">{t('statsRecords')}</div>
                ...
                <div className="rounded-2xl border border-amber-100 bg-white/90 px-4 py-3 shadow-sm shadow-amber-500/10">
                  <div className="text-xs font-medium text-slate-500">{t('statsAvgTravel')}</div>
                  <div className="mt-1 text-xl font-bold text-slate-900">
                    {globalStats?.avgTravelDays ? `${globalStats.avgTravelDays.toFixed(1)} ${t('days')}` : "-"}
                  </div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white/90 px-4 py-3 shadow-sm shadow-slate-500/5 col-span-2 sm:col-span-1">
                  <div className="text-xs font-medium text-slate-500">{t('statsCountries')}</div>
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className="mt-8 flex items-center justify-center gap-3">
          ...
          <div className="flex items-center gap-2 rounded-full border border-orange-100 bg-white px-4 py-2 text-sm shadow-sm shadow-orange-500/10">
            ...
            <span className="text-slate-500">{t('pageOf', { total: pagination.totalPages })}</span>
          </div>
...
function EmptyTabContent({ type }: { type: "timeline" | "wordcloud" | "highlights"; title: string }) {
  const t = useTranslations('SentReply');
  const iconConfig = {
    timeline: List,
    wordcloud: Cloud,
  ...
  const typeLabel = type === 'wordcloud' ? t('tabWordcloud') : t('tabHighlights');
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="rounded-[28px] border border-white/70 bg-white/90 p-8 text-center shadow-xl shadow-slate-900/5">
        ...
        <h3 className="mb-2 text-lg font-bold text-slate-900">{t('noData', { type: typeLabel })}</h3>
        <p className="mb-4 text-sm text-slate-500">{t('noDataDesc', { type: typeLabel })}</p>
        <div className="inline-flex items-center gap-2 text-sm text-slate-400">
          <List className="h-4 w-4" />
          <span>{t('switchToTimeline')}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-orange-50/30 flex items-center justify-center">
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-600">{t('loadingTimeline')}</p>
      </div>
    </div>
  );
}
      ...
      <div className="flex-1">
        ...
        {activeTab === "wordcloud" &&
          (hasData ? (
            ...
            <EmptyTabContent type="wordcloud" title="" />
          ))}
        ...
        {activeTab === "highlights" &&
          (hasData ? (
            <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
              <div className="rounded-[28px] border border-white/70 bg-white/90 p-5 shadow-xl shadow-slate-900/5">
                <h3 className="mb-4 flex items-center gap-2 text-base font-bold text-slate-900">
                  <MessageSquare className="h-5 w-5" /> {t('tabHighlights')}
                </h3>
                <FallbackScoreNotice />
            ...
            <EmptyTabContent type="highlights" title="" />
          ))}
      </div>
