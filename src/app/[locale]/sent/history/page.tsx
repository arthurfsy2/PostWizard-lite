...
export default function HistoryPage() {
  const t = useTranslations('SentHistory');
  ...
  const stats = [
    {
      label: t('statHistoryTotal'),
      value: total,
      helper: t('statHistoryHelper'),
      icon: FileText,
              ...
              <div className="grid min-w-full gap-3 sm:grid-cols-2 xl:min-w-[360px] xl:max-w-[380px] xl:grid-cols-1">
                <div className="rounded-2xl border border-orange-100 bg-gradient-to-br from-orange-50 to-amber-50 p-4 shadow-sm">
                  ...
                  <div className="space-y-2 text-sm text-slate-600">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-medium text-slate-900">{country || t('allCountries')}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="space-y-2 text-sm text-slate-600">
                    <div className="flex items-center justify-between gap-3">
                      <span>已选择</span>
                      <span className="font-medium text-slate-900">{t('selectedCount', { count: selectedIds.size })}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span>{t('availableAction')}</span>
                      <span className="font-medium text-slate-900">
                        {selectedIds.size > 0 ? t('actionExportPrint') : t('selectHint')}
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <Card className="mt-8 overflow-hidden rounded-[24px] border border-slate-200/80 bg-white/95 shadow-lg shadow-slate-200/40">
          ...
          <CardContent className="p-6">
            <form onSubmit={handleSearch} className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_minmax(0,1fr)_auto]">
              <div className="space-y-2">
                <Label htmlFor="country" className="text-sm font-medium text-slate-700">{t('countryLabel')}</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="startDate" className="text-sm font-medium text-slate-700">{t('startDate')}</Label>
                <Input
                  id="startDate"
                  type="date"
                  value={startDate}
              ...
              <div className="space-y-2">
                <Label htmlFor="endDate" className="text-sm font-medium text-slate-700">{t('endDate')}</Label>
                <Input
                  id="endDate"
                  type="date"
                  value={endDate}
        ...
        <section className="mt-6 flex flex-col gap-4 rounded-[24px] border border-slate-200/80 bg-white/90 p-5 shadow-lg shadow-slate-200/30 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {selectedIds.size > 0 ? (
              ...
              <span className="text-sm text-slate-500">{t('selectHint')}</span>
            )}
          </div>
        ...
        {!loading && history.length === 0 && (
          <Card className="mt-8 overflow-hidden rounded-[24px] border border-dashed border-slate-300 bg-white/90 shadow-lg shadow-slate-200/20">
            <CardContent className="flex flex-col items-center justify-center px-6 py-16 text-center">
              ...
              <h3 className="mt-5 text-xl font-semibold tracking-[-0.01em] text-slate-900">{t('emptyTitle')}</h3>
        ...
        {!loading && history.length > 0 && (
          <div className="mt-8 space-y-4">
            <div className="flex items-center gap-3 px-1 text-sm text-slate-500">
              ...
              <span>{t('resultsCount', { count: history.length })}</span>
            </div>
