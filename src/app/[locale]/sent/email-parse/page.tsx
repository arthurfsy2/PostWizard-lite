...
export default function EmailParsePage() {
  const t = useTranslations('SentEmailParse');
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (emails.length === 0) {
          toast.info(t('noEmailsFound'));
        } else {
          toast.success(t('count', { count: emails.length }));
        }
      } else {
        toast.error(result.error || t('searchFailed'));
      }
    } catch (error: any) {
      toast.error(t('searchFailed') + "：" + (error.message || "未知错误"));
    }
  };
  ...
  const handleParseAndRedirect = async (email: Email) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 402) {
          toast.error(t('upgradeRequired'), { description: data.error });
          return;
        }
  ...
  const handleEmailSetupSuccess = async () => {
    await refetchConfigs();
    toast.success(t('emailSetupSuccess'));
  };
            ...
            <div className="flex items-center gap-3">
              ...
              {hasConfig ? (
                ...
                <Badge variant="outline" className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-red-50 text-red-700 border-red-200 font-medium">
                  ...
                  <span className="text-sm">{t('emailNotConfigured')}</span>
                </Badge>
              )}
            ...
            <div className="bg-white rounded-xl shadow border border-slate-200/60 p-4 mb-4 transition-all duration-300 cubic-bezier(0.4, 0, 0.2, 1)">
              ...
              <div className="flex-1 min-w-[200px] relative">
                ...
                <div className="flex items-center gap-2">
                  <Select value={sortBy} onValueChange={(v) => setSortBy(v as any)}>
                    ...
                    <SelectContent className="bg-white border border-gray-200 shadow-lg">
                      ...
                      <SelectItem value="from">{t('sortSender')}</SelectItem>
                      <SelectItem value="subject">{t('sortSubject')}</SelectItem>
                    </SelectContent>
                  </Select>
                  <Button
                    variant="outline"
                ...
                <div className="text-sm text-slate-500">
                  {t('count', { count: filteredResults.length })}
                  {searchQuery && ` (${t('filteredCount', { count: searchResults.length })})`}
                </div>
              </div>
            </div>
          )}
          ...
          {!searchMutation.isPending && filteredResults.length === 0 && (
            <div className="bg-white rounded-2xl border p-8 text-center shadow-lg">
              ...
              <h3 className="text-lg font-bold text-slate-900 mb-2">
                {hasConfig ? t('noResults') : t('configureEmail')}
              </h3>
                  ...
                  >
                    ...
                    >
                        ...
                        <div className="flex items-start gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-semibold text-slate-800 text-base truncate">
                              {postcardId || email.subject || t('noSubject')}
                            </h4>
                            {postcardId && (
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100">
                        <span className="text-xs text-slate-400">{t('clickToParse')}</span>
                        {isArrived ? (
              ...
              {searchDialogStep === "config" ? (
                hasConfig ? (
                  <>
                    ...
                    <span>{t('searchDialogTitle')}</span>
                  </>
                ) : (
                  <>
                    <Mail className="w-5 h-5" />
                    <span>{t('configureEmailTitle')}</span>
                  </>
                )
              ) : (
                <>
                ...
                {!hasConfig && (
                  <div className="bg-amber-50 rounded-lg p-3 border border-amber-200 text-sm text-amber-800">
                    <div className="flex items-start gap-2">
                      ...
                      <span>{t('configureEmailDesc')}</span>
                    </div>
                  </div>
                )}
                <EmailConfigForm
              ...
              <div className="space-y-3">
                {searchResults.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-slate-500">{t('noEmailsFound')}</p>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setSearchDialogStep("config")}
                  ...
                  <>
                    <div className="text-sm text-slate-500">
                      {t('foundEmails', { count: searchResults.length })}
                    </div>
            ...
            {previewLoading ? (
              ...
              <div className="space-y-4">
                <div className="rounded-lg bg-slate-50 p-4 border border-slate-200">
                  <div className="text-sm text-slate-500 mb-1">{t('subject')}</div>
