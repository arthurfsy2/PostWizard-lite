...
export default function BatchUploadPage() {
  ...
  const t = useTranslations("ReceivedBatch");
  ...
  const [aiReady, setAiReady] = useState<boolean | null>(null); // null = 检查中
      ...
      for (const file of Array.from(files)) {
        ...
        reader.onload = (e) => {
          setQueue((prev) =>
            prev.map((q) =>
              q.id === itemId
                ? { ...q, previewUrl: e.target?.result as string }
                : q,
            ),
          );
        };
  ...
  const retryItem = async (item: QueueItem) => {
    ...
    try {
      const res = await fetch("/api/received-cards/batch", {
        method: "POST",
        body: formData,
      });
    ...
    setQueue((prev) => {
      ...
      if (data.status === "duplicate") {
        updated[idx] = {
          ...item,
          status: "duplicate",
          result: { duplicateInfo: data.duplicateInfo },
        };
        ...
        updated[idx] = {
          ...item,
          status: "done",
          result: { card: data.card, gacha: data.gacha },
        };
      }
      return updated;
    });
  ...
  const startUpload = async () => {
      ...
      prev.map((q) => {
        ...
        if (precheckDuplicateItemIds.has(q.id)) {
          return {
            ...
            status: "duplicate",
            result: {
              duplicateInfo: { postcardId: extractIdFromFilename(q.file.name) },
            },
          };
        }
    ...
    try {
      ...
      setQueue((prev) =>
        prev.map((q) =>
          q.status === "uploading"
            ? { ...q, status: "error" as const, error: err.message }
            : q,
        ),
      );
    }
  ...
  // 检测是否有 OCR 配额耗尽的卡片
  const hasOcrQuotaExhausted = queue.some(
    (q) => q.result?.card?.ocrQuotaExhausted,
  );
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-lg font-bold text-slate-900">
                    {t("uploadQueue", { count: queue.length })}
                    {doneItems.length > 0 && (
                      <span className="ml-2 text-sm text-emerald-600">
                        {t("doneCount", { count: doneItems.length })}
                      </span>
                    )}
                    {queue.filter((q) => q.status === "duplicate").length >
                      0 && (
                      <span className="ml-2 text-sm text-amber-600">
                        {t("skipped", { count: queue.filter((q) => q.status === "duplicate").length })}
                      </span>
                    )}
                    {errorItems.length > 0 && (
                      <span className="ml-2 text-sm text-red-600">
                        {t("failed", { count: errorItems.length })}
                      </span>
                    )}
                  </h2>
                  {progress && (
                ...
                <div className="flex gap-2">
                  ...
                  {pendingItems.length > 0 && !uploading && (
                    <button
                      ...
                      className="px-6 py-2 text-sm bg-orange-500 text-white rounded-lg font-medium hover:bg-orange-600 transition-all shadow-lg inline-flex items-center gap-1 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <Upload className="w-4 h-4" />
                      {t("startUpload", { count: pendingItems.length })}
                    </button>
                  )}
                </div>
              </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.status === "done" && item.result?.gacha && (
                        <div className="flex items-center gap-2 mt-1">
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full font-bold bg-gradient-to-r ${RARITY_COLORS[item.result.gacha.rarity] || RARITY_COLORS.N} text-white`}
                          >
                            {item.result.gacha.rarity}
                          </span>
                          <span className="text-xs text-slate-600">
                            {item.result.gacha.cardName}
                          </span>
                          {item.result.gacha.aiEvaluation && (
                            <span className="text-xs text-slate-500">
                              {item.result.gacha.aiEvaluation.touchingScore +
                                item.result.gacha.aiEvaluation.emotionalScore +
                                item.result.gacha.aiEvaluation
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedCard({
                          ...
                          aiEvaluation: gacha.aiEvaluation || {
                            summary: "",
                            touchingScore: 0,
                            emotionalScore: 0,
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-bold bg-gradient-to-r ${RARITY_COLORS[gacha.rarity] || RARITY_COLORS.N} text-white`}
                        >
                          {gacha.rarity}
                        </span>
                        {gacha.aiEvaluation && (
                          <span className="text-xs font-bold text-slate-700">
                            {gacha.aiEvaluation.touchingScore +
                              gacha.aiEvaluation.emotionalScore +
                              gacha.aiEvaluation.culturalInsightScore}
                          </span>
                        )}
                      </div>
